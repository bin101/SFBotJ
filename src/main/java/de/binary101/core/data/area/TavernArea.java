...
public class TavernArea extends BaseArea {
	...
	public void performArea() {
		...
		if (!account.getHasRunningAction()
			...
			if (maxBeerToBuy > 0) {
				...
				if (usedBeer < maxBeer && usedBeer < maxBeerToBuy
						&& aluSeconds <= 20 * 60) {
					while (aluSeconds + 20 * 60 < 100 * 60 && mushrooms > 1
							&& mushrooms > savedMushrooms && usedBeer < maxBeer
						...
						usedBeer += 1;
						mushrooms -= 1;
						logger.info("Habe mir ein Bier gegoennt, ist auch erst das "
								+ usedBeer + ". fuer heute.");
					}
				}
			}
			...
			if (account.getActionEndTime().isBeforeNow()
					&& account.getActionType() == ActionEnum.Quest) {
				Helper.threadSleepRandomBetween(600, 1200);
				finishQuest();
	...
	private Boolean startBestQuest(List<Quest> quests,
			Boolean overwriteInventory) {
		Boolean result = false;
		Helper.threadSleepRandomBetween(2100, 3500);

		Boolean overwriteFullInventory = true; // TODO Sollte eigentlich aus den
												// Settings gelesen werden.
			...
			try {
				chosenQuest = quests
						...
						.min((quest1, quest2) -> Integer.compare(
								quest1.getDuration(), quest2.getDuration()))
						.get();
		...
		if (!oneQuestIsSpecial) {
			...
			EventEnum currentSpecialEvent = account.getTavern()
					.getSpecialEvent();
			String settingsQuestMode = account.getSetting().getQuestMode();
			if (currentSpecialEvent == EventEnum.ExperienceEvent
					|| currentSpecialEvent == EventEnum.GoldEvent) {
				String targetQuestMode = currentSpecialEvent == EventEnum.ExperienceEvent ? "exp"
						: "gold";

				logger.info(String.format(
						"Es herrscht gerade das %s, bevorzuge daher %s",
						currentSpecialEvent, targetQuestMode));
			...
			case "gold":
				chosenQuest = quests
						...
						.max((quest1, quest2) -> Double.compare(
								quest1.getSilverPerSecond(),
								quest2.getSilverPerSecond())).get();
				break;
			default:
			...
			logger.info(String.format(
					"Habe den Questmode ignoriert, da Quest %s %s", chosenQuest
							.getIndex(), chosenQuest.getSpecialQuestType()
							.toString()));
		}

		if (chosenQuest != null) {
			responseString = sendRequest(new QuestStartRequest(
					chosenQuest.getIndex(), overwriteFullInventory));
			Response response = new Response(responseString, account);
			if (!response.getHasError()) {
				result = true;

				logger.info(String
						.format("Habe mich fuer Quest Nummer %s entschieden, werde in %.2f Minute/n wieder da sein",
								chosenQuest.getIndex(),
								(double) chosenQuest.getDuration() / 60));
	...
	private Boolean buyBeer() {
		Boolean result = false;
		String responseString = this.sendRequest(new Request(
				RequestEnum.BuyBeer));
		Response response = new Response(responseString, account);
