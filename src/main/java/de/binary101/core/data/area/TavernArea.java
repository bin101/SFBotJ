...
public class TavernArea extends BaseArea {
	...
	public void performArea() {
		if (!account.getSetting().getPerformQuesten() || account.getOwnCharacter().getBackpack().getIsFull()) {
			return;
		}
		...
		if (!account.getHasRunningAction() && account.getHasEnoughALUForOneQuest()) {
			...
			if (maxBeerToBuy > 0) {
				...
				if (usedBeer < maxBeer && usedBeer < maxBeerToBuy && aluSeconds <= 20 * 60) {
					while (aluSeconds + 20 * 60 < 100 * 60 && mushrooms > 1 && mushrooms > savedMushrooms
							&& usedBeer < maxBeer && usedBeer < maxBeerToBuy) {
	...
	private Boolean startBestQuest(List<Quest> quests, Boolean overwriteInventory) {
		...
		if (account.getSetting().getPreferSpecialQuests()) {
			try {
				chosenQuest = quests.stream()
						.filter(quest -> quest.getDuration() <= remainingAluSeconds && quest.getIsSpecial())
						.min((quest1, quest2) -> Integer.compare(quest1.getDuration(), quest2.getDuration())).get();
		...
		if (!oneQuestIsSpecial) {
			...
			if (currentSpecialEvent == EventEnum.ExperienceEvent || currentSpecialEvent == EventEnum.GoldEvent) {
				...
				logger.info(String.format("Es herrscht gerade das %s, bevorzuge daher %s", currentSpecialEvent,
						targetQuestMode));
			...
			switch (account.getSetting().getQuestMode()) {
				case "exp":
					chosenQuest = quests.stream().filter(quest -> quest.getDuration() <= remainingAluSeconds)
							.max((quest1, quest2) -> Integer.compare(quest1.getExp(), quest2.getExp())).get();
					break;
				case "gold":
					chosenQuest = quests
							.stream()
							.filter(quest -> quest.getDuration() <= remainingAluSeconds)
							.max((quest1, quest2) -> Double.compare(quest1.getSilverPerSecond(),
			...
			logger.info(String.format("Habe den Questmode ignoriert, da Quest %s %s", chosenQuest.getIndex(),
					chosenQuest.getSpecialQuestType().toString()));
		}
		...
		if (chosenQuest != null) {
			...
			if (!response.getHasError()) {
				...
				logger.info(String.format(
						"Habe mich fuer Quest Nummer %s entschieden, werde in %.2f Minute/n wieder da sein",
						chosenQuest.getIndex(), (double) chosenQuest.getDuration() / 60));
